import type { RimmelComponent, Inputs } from 'rimmel';
import { BehaviorSubject } from 'rxjs';
import { Rimmel_Bind_Subtree } from 'rimmel';
 ...
 * - Converts React props to BehaviorSubject observables (lazily, only when accessed)
...
export function WrapForReact<P extends Record<string, any> = Record<string, any>>(
  ...
  return React.forwardRef<HTMLDivElement, P>((props, ref) => {
    ...
    const subjectsRef = React.useRef(new Map<string, BehaviorSubject<any>>());
    ...
    React.useEffect(() => {
      ...
      // Update existing subjects with new prop values
      subjects.forEach((subject, key) => {
        if (key in props) {
          subject.next(props[key]);
        }
      });
      ...
      if (!isMountedRef.current && containerRef.current) {
        ...
        // Create a Proxy that lazily creates BehaviorSubjects only when accessed
        // This addresses the maintainer's concern about creating subjects for unused props
        const inputs: Inputs = new Proxy({} as Inputs, {
          get(target, prop: string) {
            // If we already have a subject for this prop, return it
            if (subjects.has(prop)) {
              return subjects.get(prop);
            }

            // Only create a subject if the prop exists in props
            if (prop in props) {
              const subject = new BehaviorSubject(props[prop]);
              subjects.set(prop, subject);
              return subject;
            }

            // Return undefined for non-existent props
            return undefined;
          },
